...
def Generate_assertndebugCSV():
    ...
    assert_lineno_str=""
    debug_lineno_str=""
    command='grep -r --include="*.py" --include="*.c"  --include="*.pyi" -n -E  "( {3,}assert\w*)|(DEBUG_ASSERT\(.*\))|Debug|(\.debug\()" >> assertndebug.txt --exclude=main.py --exclude=assertndebug.py  --exclude="*test*.py" --exclude-dir={test,tests,testing,venv} '
